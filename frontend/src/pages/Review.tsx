...
function ReviewItem({ video, onResolved }: { video: Video; onResolved: () => void }) {
  ...
  const canSave = draft.title.trim() && draft.channel.trim();

  const save = async () => {
    if (!canSave) {
      setError("Title and channel are required to clear review.");
      return;
    }
    setSaving(true);
    setError(null);
    ...
    <div className="mx-auto max-w-4xl">
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          {items.map((v) => (
            <ReviewItem key={v.id} video={v} onResolved={load} />
          ))}
        </div>
      )}
    </div>
