...
While watching a **stream preview** (`/watch?url=…`), you can start a download at the chosen quality. When the download **completes**, Horde can hand off to the new library item so playback continues from the local file (keeping your place where possible) instead of the temporary stream.
